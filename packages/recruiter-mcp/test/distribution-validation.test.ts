...
describe("remote distribution validation", () => {
  ...
  it("validates the hosted MCP protocol path and recruiter-only tool catalog", async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      if (url.hostname === "ibxvxmfhovmththllwoi.supabase.co" && url.pathname === "/rest/v1/recruiter_mcp_session_revocation") {
  ...
  it("accepts a write-entitled catalog: applies destructive, previews read-only (CLO-83)", () => {
    // The blanket read-only rule failed every correct write-entitled catalog. Applies must carry
    // the destructive triple from register.ts; previews stay read-only; and the catalog checks must
    // accept exactly base + all 22 appended — a partial grant is a defect, not a mode.
    const actionAnnotated = ACTION_DEFINITIONS.flatMap((definition) => [
      { name: definition.previewTool, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } },
      { name: definition.applyTool, annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true } },
    ]);
    const annotations = validateRemoteToolAnnotations([
      { name: "search_my_jobs", annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } },
      ...actionAnnotated,
    ]);
    assert.equal(annotations.status, "pass");

    const misannotated = validateRemoteToolAnnotations([
      { name: ACTION_DEFINITIONS[0]!.applyTool, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } },
    ]);
    assert.equal(misannotated.status, "fail", "an apply pretending to be read-only silently costs the client its confirmation prompt");

    const actionNames = ACTION_DEFINITIONS.flatMap((definition) => [definition.previewTool, definition.applyTool]);
    const catalog = validateRemoteToolCatalog([...PILOT_TOOL_NAMES, ...actionNames], [...PILOT_TOOL_NAMES]);
    for (const check of catalog) assert.equal(check.status, "pass", check.name);
    assert.equal(catalog.find((check) => check.name === "exact_tool_catalog")?.details?.catalog, "write_entitled");

    const partial = validateRemoteToolCatalog(
      [...PILOT_TOOL_NAMES, ...ACTION_DEFINITIONS.slice(1).flatMap((definition) => [definition.previewTool, definition.applyTool])],
      [...PILOT_TOOL_NAMES]
    );
    assert.equal(partial.find((check) => check.name === "exact_tool_catalog")?.status, "fail", "a partial grant is a defect, not a mode");
  });
