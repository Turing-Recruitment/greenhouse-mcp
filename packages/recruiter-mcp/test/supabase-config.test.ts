...
describe("canonical Supabase project ref enforcement", () => {
  it("exposes the canonical project constants", () => {
    assert.equal(CANONICAL_SUPABASE_PROJECT_REF, "ibxvxmfhovmththllwoi");
    assert.equal(CANONICAL_SUPABASE_REST_ORIGIN, CANONICAL);
  });

  it("extracts the project ref from a Supabase REST origin", () => {
    assert.equal(extractSupabaseProjectRef(CANONICAL), "ibxvxmfhovmththllwoi");
    assert.equal(extractSupabaseProjectRef(ANALYTICS), "ilkbfyubwvbpsevybsfe");
  ...
  it("returns undefined for hosts that are not Supabase project origins", () => {
    ...
    assert.equal(extractSupabaseProjectRef("https://ibxvxmfhovmththllwoi.supabase.co.evil.com"), undefined);
  ...
  it("rejects the recruiting-ops-analytics project by name", () => {
    assert.throws(
      ...
      (error: unknown) => {
        ...
        assert.match(message, /ilkbfyubwvbpsevybsfe/);
        assert.match(message, /ibxvxmfhovmththllwoi/);
        return true;
      },
    );
  });
  ...
  it("surfaces the URL shape error first for a malformed URL (ref check never reached)", () => {
    ...
    assert.throws(() => assertCanonicalSupabaseProjectRef("http://ibxvxmfhovmththllwoi.supabase.co", "X"), /valid HTTPS origin/);
  });
});
