...
describe("rollout evidence initializer", () => {
  it("writes a red-by-default evidence scaffold for durable desktop access", async () => {
    ...
    for (const desktop of [chatgptDesktop, claudeDesktop, claudeCode]) {
      ...
      assert.equal(desktop.catalogAttestation, "");
    ...
    // The deployment notes must describe the GCS object audit backend alternative accurately.
    assert.match(runbook, /GREENHOUSE_RECRUITER_AUDIT_BACKEND=gcs_object/);
    assert.match(runbook, /GREENHOUSE_RECRUITER_AUDIT_GCS_BUCKET/);
    assert.match(runbook, /one create-only JSON object per audit event/);
