...
describe("audit schema v2", () => {
  ...
  it("logs a name-only stderr note when the terminal audit emit converts to AUDIT_UNAVAILABLE", async () => {
    const reader = fakeScopedReader((toolName) => scopedSuccess(toolName, []));
    const { runtime, auditSink } = testRuntime(reader);
    const originalEmit = auditSink.emit.bind(auditSink);
    auditSink.emit = async (event) => {
      if (event.auditStage === "terminal") {
        const failure = new Error("bucket outage detail that must never reach stderr");
        failure.name = "FakeStorageOutageError";
        throw failure;
      }
      await originalEmit(event);
    };
    ...
    let result;
    try {
      result = await runScopedTool(runtime, "search_my_jobs", "list_jobs", {}, "evidence");
    } finally {
    ...
    // The cause's NAME identifies the failure class for the operator; the message never lands.
    assert.match(logText, /\[greenhouse-recruiter-mcp\] audit emit failed error_name=FakeStorageOutageError/);
    assert.doesNotMatch(logText, /bucket outage detail/);
  });
