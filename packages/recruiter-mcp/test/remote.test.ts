...
import { partitionAuditPathByMonth } from "../src/audit.js";
import {
  extractBearerToken,
...
describe("remote MCP authorization", () => {
  ...
  it("logs a name-only stderr note when the audit preflight 503s, without touching the wire response", async () => {
    const token = createSignedSessionToken(chatgptSession, STRONG_SESSION_SECRET);
    const req = new FakeRequest(`Bearer ${token}`);
    const res = new FakeResponse();
    ...
    try {
      await withRevocationLookup([], async () => {
        await handleRemoteMcpRequest(req as any, res as any, remoteAuthEnv({
          GREENHOUSE_RECRUITER_AUDIT_JSONL_PATH: "audit.jsonl", // relative: the preflight rejects it
        }) as NodeJS.ProcessEnv, {
          jsonrpc: "2.0",
          id: "req-audit-logline",
          method: "tools/list",
        });
      });
    } finally {
    ...
    const body = JSON.parse(res.body) as { error: { data: { denialCode: string } } };
    assert.equal(res.statusCode, 503);
    assert.equal(body.error.data.denialCode, "AUDIT_UNAVAILABLE");
    const logText = logs.join("\n");
    // The operator gets the failure CLASS on stderr (the cause's error name); the wire answer and
    // the log both stay free of messages, paths, and config values.
    assert.match(logText, /\[greenhouse-recruiter-mcp\] audit preflight failed error_name=Error/);
    assert.doesNotMatch(logText, /audit\.jsonl|absolute path|AUDIT_JSONL_PATH/);
  });
  ...
  it("denies hosted MCP requests when retained audit storage is not appendable", async () => {
    ...
    // The retained sink month-partitions the write target, so block THAT path (a directory in its
    // place makes the append fail EISDIR) rather than the unwritten base name.
    await mkdir(partitionAuditPathByMonth(auditPath, new Date()));
    const token = createSignedSessionToken(chatgptSession, STRONG_SESSION_SECRET);
    const req = new FakeRequest(`Bearer ${token}`);
    const res = new FakeResponse();
...
class FakeResponse extends EventEmitter {
  ...
  statusCode = 200;
  body = "";
  // Captures headers set before writeHead (the WWW-Authenticate path). Header assertions
  // against the real wire still belong to rawHttpRequest-style tests against a booted server;
  // this only keeps handleRemoteMcpRequest drivable in-process now that the 401 site calls
  // setHeader when OAuth is on.
