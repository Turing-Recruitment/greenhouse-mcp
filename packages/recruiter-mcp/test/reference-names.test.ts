...
// an archived/global id like rejection reason 4000002004 (~55% of a req's rejections) —
...
describe("referenceName — honest labels for unresolved reference ids (F2)", () => {
  ...
  it("labels an id absent from the reference set honestly, never a bare id or silent null", () => {
    assert.equal(referenceName(names, 4000002004, "reason"), "reason 4000002004 (name unavailable)");
