...
describe("action plane mount — catalog contract", () => {
  ...
  it("keeps the write plane when read analytics are disabled — actions are not 'analysis'", () => {
    // The registrar used to gate every action tool through isToolEnabled(kind: "analysis"), so the
    // read-plane analytics switch silently removed all 22 write tools while health stayed green
    // (CLO-183). The contract lives beside isToolEnabled in limits.ts: the category switches are
    // read-plane concepts, and disabling the analyzers says nothing about writes.
    const names: string[] = [];
    const runtime = createRecruiterRuntimeForServer({
      session: session(),
      env: { ...BASE_ENV, GREENHOUSE_RECRUITER_DISABLE_ANALYTICS: "true" },
      configureGreenhouse: false,
      scopedReader: { async scopedRead() { throw new Error("no read in a catalog test"); } },
      actionPlane: stubMount(),
    });
    registerRecruiterTools({ tool(name) { names.push(name); } }, runtime);

    assert.equal(names.includes("analyze_scorecard_accountability"), false,
      "the switch must still disable the analyzers themselves");
    for (const definition of ACTION_DEFINITIONS) {
      assert.ok(names.includes(definition.previewTool),
        `${definition.previewTool} must survive the analytics switch`);
      assert.ok(names.includes(definition.applyTool),
        `${definition.applyTool} must survive the analytics switch`);
    }
  });

  it("still lets the operator denylist beat a grant", () => {
    const denied = ACTION_DEFINITIONS[0]!.applyTool;
    const names: string[] = [];
    const runtime = createRecruiterRuntimeForServer({
      session: session(),
      env: { ...BASE_ENV, GREENHOUSE_RECRUITER_DISABLE_TOOLS: denied },
      configureGreenhouse: false,
      scopedReader: { async scopedRead() { throw new Error("no read in a catalog test"); } },
      actionPlane: stubMount(),
    });
    ...
    assert.equal(names.includes(denied), false, "a denylisted action name never registers");
    assert.ok(names.includes(ACTION_DEFINITIONS[0]!.previewTool), "the sibling preview is untouched");
  });
});
...
describe("mountActionPlane — who gets a mount at all", () => {
  ...
  it("withholds the write tools when the action runtime cannot be CONSTRUCTED, and never fails the read session", async () => {
    ...
    const { GREENHOUSE_ACTION_SUPABASE_URL: _u, GREENHOUSE_ACTION_SUPABASE_KEY: _k, ...envMissingRuntime } = {
      ...ENV,
      GREENHOUSE_ACTION_SUPABASE_URL: "https://ibxvxmfhovmththllwoi.supabase.co",
