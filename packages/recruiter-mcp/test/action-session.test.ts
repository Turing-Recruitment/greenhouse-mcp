...
describe("action session bridge", () => {
  ...
  it("refuses a session it could not revoke", () => {
    assert.throws(
  ...
  it("infers the client from the signed surface for a legacy desktop token", () => {
    // Sam's call, 2026-08-06 (CLO-103): every distributed token predates the client claim, and
    // re-issuing them means hand-holding recruiters through a reinstall. The surface is inside the
    // HMAC, so this fallback reads signed data — and the read plane already serves these tokens.
    const derived = deriveActionSession({
      session: recruiterSession({ client: undefined }),
      signingSecret: SECRET,
    });
    assert.equal(derived.session.client, "claude_desktop_chat");
    assert.equal(derived.clientAttribution, "inferred_from_surface");
  });

  it("infers codex for a legacy chatgpt token", () => {
    const derived = deriveActionSession({
      session: recruiterSession({ client: undefined, surface: "chatgpt_desktop" }),
      signingSecret: SECRET,
    });
    assert.equal(derived.session.client, "codex");
    assert.equal(derived.clientAttribution, "inferred_from_surface");
  });

  it("marks a signed client as signed, never inferred", () => {
    const derived = deriveActionSession({ session: recruiterSession(), signingSecret: SECRET });
    assert.equal(derived.session.client, "claude_desktop_chat");
    assert.equal(derived.clientAttribution, "signed");
  });

  it("still refuses write authority to a test-surface session", () => {
    assert.throws(
      () => deriveActionSession({ session: recruiterSession({ client: undefined, surface: "test" }), signingSecret: SECRET }),
      (error: unknown) => error instanceof ActionSessionBridgeError && error.code === "UNSUPPORTED_CLIENT"
    );
  });
