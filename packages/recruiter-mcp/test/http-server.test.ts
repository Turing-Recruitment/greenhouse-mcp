...
describe("hosted recruiter MCP HTTP server", () => {
  ...
  it("denies a revoked token before read_my_resume can fetch attachment metadata or bytes", async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      if (url.startsWith("https://ibxvxmfhovmththllwoi.supabase.co/rest/v1/recruiter_mcp_session_revocation")) {
  ...
  it("gates /mcp on the GCS audit backend: a failing store 503s, a working store serves and retains objects", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async (input, init) => {
      const url = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
      if (url.includes("/rest/v1/recruiter_mcp_session_revocation")) {
    ...
    // ONE factory for the whole test, installed up front: the first load rejects (the exact Cloud
    // Run outage class), every later load returns the working fake — the first-throw-then-succeed
    // pattern. Phase 1's 503 and Phase 2's recovery therefore exercise the REAL never-memoize-
    // failure path: re-installing the factory between phases would call the test-state reset,
    // wiping the probe memo and keeping Phase 2 green even if the code memoized failures.
    let storageLoads = 0;
    setGcsStorageFactoryForTests(async () => {
      storageLoads += 1;
      if (storageLoads === 1) throw new Error("gcs client load refused");
      return fake.storage;
    });
    const server = await startHttpRecruiterMcp({
      GREENHOUSE_RECRUITER_MCP_PORT: "0",
      ...
      GREENHOUSE_RECRUITER_REVOCATION_SUPABASE_URL: "https://ibxvxmfhovmththllwoi.supabase.co",
      GREENHOUSE_RECRUITER_REVOCATION_SUPABASE_KEY: "revocation-key-value",
      // No JSONL path and no durable mount: the GCS backend carries the audit gate alone.
      GREENHOUSE_RECRUITER_AUDIT_BACKEND: "gcs_object",
      GREENHOUSE_RECRUITER_AUDIT_GCS_BUCKET: "integration-audit-bucket",
    } as NodeJS.ProcessEnv);
    try {
      const listBody = () => JSON.stringify({ jsonrpc: "2.0", id: "gcs-gate", method: "tools/list" });
      const denied = await fetch(`${baseUrl(server)}/mcp`, {
        method: "POST",
        headers: mcpHeaders(signedRemoteToken()),
        body: listBody(),
      });
      const deniedText = await denied.text();
      const deniedBody = JSON.parse(deniedText) as { error: { data: { denialCode: string } } };
      assert.equal(denied.status, 503);
      assert.equal(deniedBody.error.data.denialCode, "AUDIT_UNAVAILABLE");
      assert.doesNotMatch(deniedText, /gcs client load refused|integration-audit-bucket/);

      // Phase 2: the store recovers (the factory now yields the working fake). Failure was never
      // memoized, so the SAME server — with no harness reset in between — serves and probes now.
      const served = await fetch(`${baseUrl(server)}/mcp`, {
        method: "POST",
        headers: mcpHeaders(signedRemoteToken()),
        body: listBody(),
      });
      ...
      const probeSaves = fake.saves.filter((save) => save.path.includes("/.probe/"));
      assert.equal(probeSaves.length, 1, "serving requires one real probe write");
      assert.match(probeSaves[0]!.path, /^audit\/\.probe\//);
      assert.deepEqual(fake.deletes, [probeSaves[0]!.path], "the probe object is best-effort deleted");

      // Phase 3: a real tool call retains create-only, contract-named, metadata-only objects.
      const called = await fetch(`${baseUrl(server)}/mcp`, {
        method: "POST",
        headers: mcpHeaders(signedRemoteToken()),
        body: JSON.stringify({
          jsonrpc: "2.0",
      ...
      const eventObjectNames = [...fake.objects.keys()].filter((name) => !name.includes("/.probe/"));
      assert.ok(eventObjectNames.length >= 1, "a tool call must retain at least one audit object");
      for (const name of eventObjectNames) {
        assert.match(name, /^audit\/\d{4}-\d{2}\/\d{8}T\d{9}Z-[A-Za-z0-9._:-]+-(start|terminal)\.json$/);
        const event = JSON.parse(fake.objects.get(name)!) as { event: string; tool: string };
        assert.equal(event.event, "scoped_greenhouse_tool_call");
        assert.equal(event.tool, "search_my_jobs");
      }
      const eventSaves = fake.saves.filter((save) => !save.path.includes("/.probe/"));
      assert.ok(eventSaves.every((save) =>
        save.options.preconditionOpts?.ifGenerationMatch === 0 && save.options.resumable === false
      ), "every retained audit object is written create-only and non-resumable");

      // Phase 4: the process-lifetime success memo holds — a further request serves without
      // re-probing this target for the rest of the process.
      const again = await fetch(`${baseUrl(server)}/mcp`, {
        method: "POST",
        headers: mcpHeaders(signedRemoteToken()),
        body: listBody(),
      });
      ...
      assert.equal(fake.saves.filter((save) => save.path.includes("/.probe/")).length, 1);
    } finally {
      setGcsStorageFactoryForTests(null);
      globalThis.fetch = originalFetch;
      // This test's search_my_jobs SUCCEEDS, which primes the module-scoped upstream read cache;
      // later tests latch on a real harvest fetch happening, so the cache must not leak forward.
      _resetReadCache();
      await closeServer(server);
    }
  });
  ...
  it("serves /ready as a public unauthenticated liveness probe while serving", async () => {
    const server = await startHttpRecruiterMcp({ GREENHOUSE_RECRUITER_MCP_PORT: "0" } as NodeJS.ProcessEnv);
    try {
      ...
      assert.equal(response.status, 200);
      assert.deepEqual(JSON.parse(text), { ok: true, status: "ready" });
      // The public probe is boolean-only: it must not leak the detailed readiness checks or config
      // the token-gated /readyz surface exposes.
      assert.doesNotMatch(text, /checks|session_secret|supabase/i);
    } finally {
      await closeServer(server);
    }
  });

  it("rejects non-GET methods on the public /ready probe", async () => {
    const server = await startHttpRecruiterMcp({ GREENHOUSE_RECRUITER_MCP_PORT: "0" } as NodeJS.ProcessEnv);
    try {
      const response = await fetch(`${baseUrl(server)}/ready`, { method: "POST" });
      assert.equal(response.status, 405);
  ...
  it("drains in-flight reads under the shutdown deadline and flips /ready to 503", async () => {
    const dir = await mkdtemp(join(tmpdir(), "greenhouse-http-drain-"));
    const auditPath = join(dir, "audit.jsonl");
    const originalFetch = globalThis.fetch;
    let markUpstreamStarted!: () => void;
    const upstreamStarted = new Promise<void>((resolve) => { markUpstreamStarted = resolve; });
    ...
    try {
      const readyBefore = await fetch(`${baseUrl(server)}/ready`);
      assert.equal(readyBefore.status, 200, "ready while serving");
      assert.equal((await readyBefore.json() as { status: string }).status, "ready");

      inFlight = fetch(`${baseUrl(server)}/mcp`, {
        method: "POST",
        headers: mcpHeaders(signedRemoteToken()),
        body: JSON.stringify({
          jsonrpc: "2.0",
      ...
      await upstreamStarted;
      const shutdownPromise = shutdownHttpRecruiterMcp(server);

      const readyDraining = await fetch(`${baseUrl(server)}/ready`);
      assert.equal(readyDraining.status, 503, "ready flips to 503 once draining");
      assert.deepEqual(await readyDraining.json(), { ok: false, status: "not_ready" });

      await shutdownPromise;
      assert.equal(upstreamAborted, true, "the shutdown deadline force-aborts the in-flight Greenhouse read");
      assert.equal(server.listening, false, "the listener is closed after shutdown completes");
    } finally {
      cleaningUp = true;
      await inFlight;
      globalThis.fetch = originalFetch;
