...
describe("pipeline state snapshot (temporal logbook, Option B)", () => {
  ...
  it("in-service timer is DORMANT by default and starts only behind the env gate", () => {
    ...
    stopPipelineSnapshotTimer(timer);
  });

  it("suppresses the in-service timer on Cloud Run (K_SERVICE) even behind the env gate", () => {
    // Cloud Run runs multiple instances and overlapping revisions; a per-instance daily sweep
    // would double-run. Snapshot scheduling belongs to a singleton Cloud Run Job / Cloud Scheduler
    // invocation, so a serving instance must never start the timer when K_SERVICE is present.
    const timer = maybeStartPipelineSnapshotTimer(
      { GREENHOUSE_RECRUITER_SNAPSHOT_ENABLED: "true", K_SERVICE: "greenhouse-recruiter-mcp" } as NodeJS.ProcessEnv,
      async () => {}
    );
    assert.equal(timer, null, "a serving Cloud Run instance must not schedule the snapshot sweep");

    // Off Cloud Run (no K_SERVICE) the timer still starts as before.
    const offCloudRun = maybeStartPipelineSnapshotTimer(
      { GREENHOUSE_RECRUITER_SNAPSHOT_ENABLED: "true" } as NodeJS.ProcessEnv,
      async () => {}
    );
    assert.notEqual(offCloudRun, null);
    stopPipelineSnapshotTimer(offCloudRun);
  });

  it("stopPipelineSnapshotTimer tears the timer down and tolerates a null timer", () => {
    stopPipelineSnapshotTimer(null); // no-op, must not throw
    const timer = maybeStartPipelineSnapshotTimer(
      { GREENHOUSE_RECRUITER_SNAPSHOT_ENABLED: "true" } as NodeJS.ProcessEnv,
      async () => {}
    );
    assert.notEqual(timer, null);
    assert.doesNotThrow(() => stopPipelineSnapshotTimer(timer));
    assert.doesNotThrow(() => stopPipelineSnapshotTimer(timer), "double stop is idempotent");
  });
