...
describe("recruiter MCP rate limiting", () => {
  ...
  it("keys OAuth sessions on the durable sid so rotating the access-token jti cannot reset the bucket (R1-B)", () => {
    const limiter = createInMemoryRateLimiter({
      windowMs: 60_000,
      maxCallsPerWindow: 1,
      maxAnalysisCallsPerWindow: 1,
    });
    // Two access tokens from the SAME sign-in (same grant family sid) but different jtis — exactly
    // what a refresh rotation produces. They must share one bucket, or /token becomes a free reset.
    const first = limiter.check({
      session: testSession({ sid: "grant-family-1", tokenId: "access-jti-1", surface: "claude_desktop" }),
      toolName: "search_my_jobs", toolKind: "evidence", now: 0,
    });
    const afterRotation = limiter.check({
      session: testSession({ sid: "grant-family-1", tokenId: "access-jti-2", surface: "claude_desktop" }),
      toolName: "search_my_jobs", toolKind: "evidence", now: 1,
    });
    assert.equal(first.allowed, true);
    assert.equal(afterRotation.allowed, false, "a rotated jti under the same sid must NOT get a fresh bucket");

    // Control: a different sid (a distinct sign-in) is a distinct principal and keeps its own bucket.
    const otherFamily = limiter.check({
      session: testSession({ sid: "grant-family-2", tokenId: "access-jti-3", surface: "claude_desktop" }),
      toolName: "search_my_jobs", toolKind: "evidence", now: 2,
    });
    assert.equal(otherFamily.allowed, true, "a different grant family is a different principal");
  });
