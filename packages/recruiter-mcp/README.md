...
A production container build artifact is checked in at `deploy/Dockerfile`. Build it from the `ta-ops-analytics` repo root so the image can include the Greenhouse client package, the additive scoped core, and this recruiter-only HTTP entrypoint:
...
Before a pilot or broader distribution, run the whole workspace from the repository root:

```sh
npm run verify
```

That builds the control plane, then runs every package's typecheck, tests, and static guardrails — including the proof that the recruiter runtime source plus command entrypoints stayed read-only and that raw Greenhouse client usage is isolated to `src/scoped-reader.ts`. (In the `ta-ops-analytics` monorepo this was `npm run verify:rollout`, which walked the surrounding tree by relative path; the workspace root replaces it.)
