...
function rateLimitPrincipalKey(session: AuthenticatedSession): string {
  // Prefer the durable per-session id (the OAuth grant family) so rotating a refresh token does
  // NOT hand the caller a fresh bucket every /token — the Greenhouse-quota limit has to bind to
  // the sign-in, not the ephemeral access-token jti. Legacy tokens leave sid unset and stay on
  // their immortal tokenId; both fall back to the subject only when neither is present.
  const principal = session.sid ?? session.tokenId ?? session.subject;
