...
export async function emitRequiredToolAudit(
  ...
  try {
    ...
    if (isAuditUnavailableError(error)) {
      // Name-only operator breadcrumb for the fail-closed denial: the CAUSE's error name when
      // present — never the message, never config values.
      const causeName = error.cause instanceof Error ? error.cause.name : error.name;
      console.error(`[greenhouse-recruiter-mcp] audit emit failed error_name=${causeName}`);
