 ...
 * Greenhouse-global id the org's current list omits (e.g. rejection reason 4000002004,
 * ~55% of a req's rejections). Never a bare id or a silent null; null only when there is
 * no id at all. `kind` is the reference noun ("source", "referrer", "reason") shown in
 * the label so a recruiter reads "reason 4000002004 (name unavailable)" rather than a
 * bare number.
 */
