...
// Maps each daily interval timer to its paired startup-delay timer so stopPipelineSnapshotTimer can
// tear both down on graceful shutdown; clearing the interval alone would leak the pending startup sweep.
const startupSnapshotTimers = new WeakMap<NodeJS.Timeout, NodeJS.Timeout>();
...
export function maybeStartPipelineSnapshotTimer(
  ...
  // Cloud Run may run multiple instances and overlapping revisions, so a per-instance daily sweep
  // would double-run. Snapshot scheduling belongs to a singleton Cloud Run Job / Cloud Scheduler
  // invocation, never a serving instance — K_SERVICE is populated only inside the Cloud Run runtime.
  if (env.K_SERVICE?.trim()) return null;
  ...
  timer.unref?.();
  startupSnapshotTimers.set(timer, startupDelay);
  console.error("[greenhouse-recruiter-mcp] pipeline snapshot timer enabled (daily refresh, weekly upsert key)");
  return timer;
}

// Graceful-shutdown counterpart to maybeStartPipelineSnapshotTimer: clears the daily interval and the
// paired startup-delay timer. Null-tolerant (the timer is null whenever the sweep is dormant) and
// idempotent (a second call is a no-op once the pairing is removed).
export function stopPipelineSnapshotTimer(timer: NodeJS.Timeout | null): void {
  if (!timer) return;
  clearInterval(timer);
  const startupTimer = startupSnapshotTimers.get(timer);
  if (startupTimer) clearTimeout(startupTimer);
  startupSnapshotTimers.delete(timer);
}
