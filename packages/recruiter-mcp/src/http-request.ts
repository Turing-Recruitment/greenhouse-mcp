...
// Fixed literal routes the server serves OUTSIDE the env-configured trio: the public /version
// build-identity route, the public /ready Cloud Run readiness probe (both in http-server.ts), and
// the OAuth discovery + endpoint routes under /.well-known/ (oauth-metadata.ts). The env-configured
// paths must never shadow them — before this check existed, GREENHOUSE_RECRUITER_MCP_PATH=/version
// booted a server whose /mcp surface was silently unreachable (the literal branch won the if-chain),
// despite http-server.ts claiming the distinctness check prevented exactly that. /ready joins the
// reservation for the same reason: Cloud Run polls it for startup/liveness, so a misconfigured
// health or readiness route must not be allowed to steal it.
export const VERSION_ROUTE_PATH = "/version";
// Fixed, unauthenticated Cloud Run readiness route (http-server.ts). Reserved so no env-configured
// path can shadow the probe the platform uses to decide an instance is serving / draining.
export const PUBLIC_READY_PATH = "/ready";
const RESERVED_WELL_KNOWN_PREFIX = "/.well-known";
// The mounted OAuth endpoint literals (oauth-metadata.ts). Kept as strings here rather than
// imports to avoid a config<->server module cycle; the oauth-metadata constants are the same
// literals and a divergence would fail the discovery/authorize test suites.
const RESERVED_OAUTH_ENDPOINT_PATHS = ["/authorize", "/oauth/callback", "/token"];

export function isReservedLiteralRoutePath(path: string): boolean {
  return (
    path === VERSION_ROUTE_PATH ||
    path === PUBLIC_READY_PATH ||
    path === RESERVED_WELL_KNOWN_PREFIX ||
    path.startsWith(`${RESERVED_WELL_KNOWN_PREFIX}/`) ||
    RESERVED_OAUTH_ENDPOINT_PATHS.includes(path)
  );
}

export function readHttpEndpointConfig(env: NodeJS.ProcessEnv = process.env): HttpEndpointConfig {
  // Cloud Run injects PORT and expects the container to honor it; the recruiter-specific fallback
  // covers Render/local, where PORT is unset. PORT wins only when present and non-empty — an empty
  // PORT ("") falls through to the fallback, while a malformed non-empty PORT surfaces as a PORT
  // error rather than being silently ignored.
  const portEnvName = env.PORT === undefined || env.PORT.length === 0
    ? "GREENHOUSE_RECRUITER_MCP_PORT"
    : "PORT";
  const config = {
    port: readHttpListenPort(env[portEnvName], portEnvName),
  ...
  const entries: Array<[string, string]> = [
    ["GREENHOUSE_RECRUITER_MCP_PATH", config.mcpPath],
    ["GREENHOUSE_RECRUITER_HEALTH_PATH", config.healthPath],
    ["GREENHOUSE_RECRUITER_READY_PATH", config.readyPath],
  ];
  for (const [envName, path] of entries) {
    if (isReservedLiteralRoutePath(path)) {
      throw new Error(
        `${envName} must not shadow the reserved server routes (${VERSION_ROUTE_PATH}, ${PUBLIC_READY_PATH}, ${RESERVED_WELL_KNOWN_PREFIX}/*, ${RESERVED_OAUTH_ENDPOINT_PATHS.join(", ")}).`
      );
    }
  }
  return config;
...
function readHttpListenPort(value: string | undefined, envName: string): number {
  if (value !== undefined && value.trim().length > 0 && value.trim() !== value) {
    throw new Error(`${envName} must be an integer from 0 to 65535.`);
  }
...
// Bounded application/x-www-form-urlencoded reader for the OAuth /token endpoint (RFC 6749
// requires form bodies there; a JSON-only parser is a named client-integration failure mode).
// Mirrors readBoundedJsonBody's byte-cap discipline exactly: 415 for the wrong media type,
// 413 past the cap, declared Content-Length checked before a byte is read.
export async function readBoundedFormBody(req: IncomingMessage, maxBytes: number): Promise<URLSearchParams> {
  validateFormContentType(req.headers["content-type"]);
  const contentLength = parseContentLength(req.headers["content-length"]);
  if (contentLength !== undefined && contentLength > maxBytes) {
    throw bodyTooLarge(maxBytes);
  }
...
function validateFormContentType(raw: string | string[] | undefined): void {
  const value = Array.isArray(raw) ? undefined : raw;
  const mediaType = value?.split(";", 1)[0]?.trim().toLowerCase();
  if (mediaType !== "application/x-www-form-urlencoded") {
    throw new HttpRequestBodyError(415, -32000, "Unsupported Media Type: Content-Type must be application/x-www-form-urlencoded");
  }
}
