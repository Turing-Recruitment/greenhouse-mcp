...
import { buildGcsAuditConfigFromEnv, readAuditBackendFromEnv } from "./audit-gcs.js";
...
export function buildRecruiterMcpReadinessReport(
  ...
  checks.push(oauthAuthorizationCheck(env));
...
function auditSinkCheck(env: NodeJS.ProcessEnv): ReadinessCheck {
  // Backend fork, same check name (`audit_sink` — the locked catalog does not grow): the GCS arm
  // shape-validates bucket/prefix (pure env math; the request preflight proves actual writability),
  // and an unrecognized backend value fails loudly naming the allowed values — it must never read
  // as "unset" and silently grade the file arm instead.
  ...
  if (backend === "gcs_object") {
    try {
      buildGcsAuditConfigFromEnv(env);
    } catch (error) {
      return {
        name: "audit_sink",
        status: "fail",
    ...
    return {
      name: "audit_sink",
      status: "pass",
      summary: "Retained GCS object-per-event audit sink is configured (inherently durable object storage; no file mount required).",
    };
  }
...
export function isAuditSinkDurable(env: NodeJS.ProcessEnv): boolean {
  // GCS arm: object storage is inherently durable, so a valid backend+bucket/prefix DECLARATION is
  // durable with no mount env. The division of labor stays: the request preflight proves WRITABLE,
  // this proves the durable declaration. An invalid backend value is never durable — it must not
  // inherit durability from otherwise-valid file env.
  let backend: ReturnType<typeof readAuditBackendFromEnv>;
  try {
    backend = readAuditBackendFromEnv(env);
  } catch {
...
// Tri-state, matching the additive contract of the OAuth sign-in layer: an entirely absent
// GREENHOUSE_RECRUITER_OAUTH_* family is a PASS (legacy-only deployments stay ready and
// byte-identical), a fully valid family is a pass with the issuer named, and a partial or
// malformed family FAILS with the parse reason — the routes are dark either way, so a half-set
// deployment is loudly broken here rather than silently legacy-only. Reasons name env vars,
// never their values.
function oauthAuthorizationCheck(env: NodeJS.ProcessEnv): ReadinessCheck {
  const result = readOauthAuthorizationConfig(env);
  if (result.state === "absent") {
    return {
      name: "oauth_authorization",
      status: "pass",
      summary: "OAuth sign-in layer is not configured; legacy bearer sessions only.",
    };
  }
  ...
  return {
    name: "oauth_authorization",
    status: "pass",
    summary: `OAuth sign-in layer is configured with issuer ${result.config.issuer} for resource ${result.config.resourceUrl}.`,
  };
}
