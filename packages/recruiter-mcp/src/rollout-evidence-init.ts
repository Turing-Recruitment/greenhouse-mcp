...
The audit sample must remain metadata-only and include success and denial events plus paired v2 terminal attribution for \`claude_desktop_chat\`, \`claude_code\`, and \`chatgpt_codex_host\`. Historical and \`legacy_unknown\` rows remain reviewable but do not satisfy physical-client coverage. On the \`gcs_object\` backend the CLI also accepts \`--from <ISO>\`/\`--to <ISO>\` or \`--months <N>\` to narrow the reviewed window, and \`--gcs-bucket <name>\`/\`--gcs-prefix <p>\` to review an explicit object-store target instead of the env-derived one; \`--dump\` (both backends) streams the assembled JSONL to stdout instead of the report.
