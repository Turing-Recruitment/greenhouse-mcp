...
// The GREENHOUSE_RECRUITER_REVOKED_TOKEN_IDS parse, exported so the OAuth access-token
// validator applies the SAME static kill list to `jti` values that this file applies to legacy
// tokenIds — one env var, one parse, both token shapes.
export function parseRevokedTokenIdList(raw: string | undefined): Set<string> {
