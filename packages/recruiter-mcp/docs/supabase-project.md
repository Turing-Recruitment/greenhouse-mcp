...
- Project ref: `ibxvxmfhovmththllwoi`
- Organization id: `mpsvxfqgofzfxiczacmk`
...
Do not use `recruiting-ops-analytics` (`ilkbfyubwvbpsevybsfe`) when checking Greenhouse MCP Supabase state. That is a different project and can contain recruiting-ops/YTD analytics tables that do not belong to this MCP.
...
`ibxvxmfhovmththllwoi`, so a misconfigured deploy fails loudly instead of silently reading the wrong
project.
...
supabase link --project-ref ibxvxmfhovmththllwoi --yes
supabase db push
```

> Do **not** relink the repo-root checkout to `ibxvxmfhovmththllwoi` and `supabase db push` from
