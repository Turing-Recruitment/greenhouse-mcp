...
The Greenhouse control plane is a read-only MCP server for asking our Greenhouse tenant operational questions from Claude Desktop, Cursor, or another MCP-compatible AI workspace. It is the UNSCOPED operator surface: it reads with the credential's full Harvest permissions and does not enforce per-user scope, so run it only if your own Greenhouse account is already entitled to everything you ask it about. Colleagues who need per-user enforcement want `packages/recruiter-mcp` instead.
...
- Read-only Greenhouse Harvest connection details for the Turing tenant, from whoever administers Greenhouse

If you do not have the Greenhouse connection details, ask our Greenhouse admin for them:

```text
I want to run the read-only Greenhouse control-plane MCP so I can ask operational
questions from Claude, Cursor, or another AI workspace. Can you create a read-only
Harvest credential scoped to my own access?
```
...
This is internal Turing code. It is not licensed for distribution outside the company, and the
credential you configure here reads real candidate data — treat the config file as a secret.
