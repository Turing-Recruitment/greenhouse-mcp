...
describe("action access operator", () => {
  ...
  test("accepts only canonical direct or pooler database URLs", () => {
    assert.equal(assertCanonicalActionDatabaseUrl(
      "postgresql://postgres:secret@db.ibxvxmfhovmththllwoi.supabase.co:5432/postgres"
    ).connection, "direct");
    assert.equal(assertCanonicalActionDatabaseUrl(
      "postgres://postgres.ibxvxmfhovmththllwoi:secret@aws-0-us-west-1.pooler.supabase.com:6543/postgres"
    ).connection, "pooler");
