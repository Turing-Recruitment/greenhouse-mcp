...
// Structural, not a count and not adjacency: there are three assertNotRevoked call sites and a
// threshold survives deleting the one that matters, while an adjacency regex outlaws legitimate
// preflight growth — Phase 2c put the visibility fence between the fresh-match and the revocation
// re-check, which is exactly where it belongs, and the old `);\s*await` pattern called that a
// violation. What must stay true is that the FRESH PREFLIGHT — the block after
// assertPreparedMatches and before beginMutation — re-checks BOTH: the fence (may this human touch
// this record, answered live from the fresh preparation) and revocation (may this session still
// act at all). A token revoked between preview and apply must not write.
const applyPreflight = serviceSource.match(/assertPreparedMatches\(intent, freshPrepared\);([\s\S]*?)beginMutation/)?.[1] ?? "";
assert(/await this\.assertTargetsVisible\(freshPrepared\.fenceTargets\)/.test(applyPreflight), "Apply's fresh preflight must run the visibility fence against the FRESH preparation's targets — a fence consulted only at preview can be outrun by a permission change.");
assert(/await this\.assertNotRevoked\(\)/.test(applyPreflight), "Revocation must be re-checked inside apply's fresh preflight — after the fresh preparation matches, before the mutation begins — not only at entry, or a token revoked between preview and apply still writes.");
