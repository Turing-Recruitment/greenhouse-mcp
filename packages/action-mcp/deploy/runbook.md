...
> **PARTIALLY ARCHIVED 2026-08-18 (CLO-93).** The standalone action service this runbook deploys
> **no longer exists** — commit `f11a509` removed the second front door, and since `68c88f5`/`2d2fe7b`
> the write plane mounts INSIDE the recruiter service: one URL, one session, one audit sink.
> `render-action.yaml` was never created. Deploy truth lives in the repo-root `render.yaml` runbook;
> the `GREENHOUSE_ACTION_*` switches live on the recruiter service (`srv-d92vprtaeets73aqcj50`) and
> the shared `Greenhouse MCP` env group, and both switches are `true` in production as of `392a69c`.
> What remains operative here: §3 entitlement provisioning (`greenhouse-action-access`), §6's
> per-capability canary discipline, §8's MANUAL reconciliation sweep, and §10's revocation CLI.
> §4–§5 describe the dead architecture and are kept for reference only — do not operate from them.
...
**No scheduled reconciler exists** — the five-minute cron definition died with the standalone
service (`f11a509`), and nothing else schedules a sweep (confirmed 2026-08-18; the scheduled home is
Cloud Run Jobs, CLO-185). Until then an apply interrupted mid-flight holds its resource lock until a
MANUAL sweep runs. The sweep reads Greenhouse and updates existing ledger rows; it never retries a
business mutation.
